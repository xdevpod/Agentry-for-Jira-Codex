...
Call the `jira_push_session` MCP tool (from the `agentry-for-jira` server). No arguments
...
- Requires the `agentry-for-jira` MCP server to be configured in `~/.codex/config.toml`

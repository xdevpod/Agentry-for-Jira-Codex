 ...
 * (agentry-for-jira) so it never touches Claude Code's own keychain entry
 * and never sits in ~/.agentry-for-jira/config.json alongside the non-secret values.
 *
...
export const SERVICE = 'agentry-for-jira';
export const LABEL = 'Agentry for Jira';
...
async function linuxSet(json) {
  ...
  await execFileP('secret-tool', ['store', `--label=${LABEL}`, 'service', SERVICE, 'account', ACCOUNT], { input: json });
}

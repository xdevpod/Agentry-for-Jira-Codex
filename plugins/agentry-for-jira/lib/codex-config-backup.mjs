 ...
 * table 路径（如 mcp_servers.agentry-for-jira.env.SOME_TOKEN）用于 dry-run 报告。
 */
...
function tablePathFromHeader(line) {
  ...
  if (!m) return null;
  // 形如 mcp_servers.agentry-for-jira.env 或 projects."/some/path"——剥掉整体外层引号（罕见）
 ...
 *  - redactedKeys 带 table 路径前缀（如 mcp_servers.agentry-for-jira.env.TOKEN）

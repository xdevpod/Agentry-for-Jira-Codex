...
if (!pluginRoot) {
  process.stderr.write(
    'jira-sync hook: could not resolve plugin root for SessionStart launcher.\n'
  );
...
child.on('error', (err) => {
  process.stderr.write(`jira-sync hook: failed to launch SessionStart check-config hook: ${err.message}\n`);
  process.exit(0);
});
